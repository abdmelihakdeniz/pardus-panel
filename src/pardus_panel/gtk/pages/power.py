...
class PowerPage:
    ...
    def _render(self, snapshot: PowerSnapshot) -> None:
        ...
        available = bool(snapshot.profiles)
        self._profile.set_sensitive(available)
        self._profile.set_visible(available)
        self._profile_unavailable.set_visible(not available)
        self._updating_profile = False
