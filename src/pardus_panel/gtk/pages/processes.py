...
PID, NAME, USER, MEMORY_RAW, CREATE_TIME, STATUS = 0, 1, 2, 5, 7, 8
...
class ProcessPage:
    ...
    def _confirm_termination(self) -> None:
        ...
        dialog = Gtk.MessageDialog(
            ...
            text=_("End {name}?").format(name=self._selected_name()),
        )
        dialog.format_secondary_text(
            _("PID {pid} will be asked to stop.").format(pid=pid)
        )
        button = dialog.add_button(_("End Process"), Gtk.ResponseType.OK)
    ...
    def _termination_done(self, status: TerminationStatus) -> None:
        ...
        self._status.set_text(TERMINATION_STATUS_LABELS[status])
