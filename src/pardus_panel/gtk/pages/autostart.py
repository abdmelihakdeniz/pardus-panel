...
class AutostartPage:
    def __init__(self, *, jobs: AsyncJobRunner) -> None:
        ...
        self._scope.connect(
            add_button, "clicked", lambda _button: self._show_add_dialog()
        )
    ...
    def _make_row(self, entry: AutostartEntry) -> Gtk.ListBoxRow:
        ...
        command = Gtk.Label(label=entry.command, xalign=0)
    ...
    def _set_enabled(self, entry: AutostartEntry, enabled: bool) -> bool:
        self._submit(lambda: self._repository.set_enabled(entry, enabled=enabled))
        return False

    def _delete(self, entry: AutostartEntry) -> None:
        self._submit(lambda: self._repository.delete(entry))
    ...
    def _show_add_dialog(self) -> None:
        ...
        self._submit(lambda: self._repository.create(name=name, command=command))

    def _submit(self, work: Callable[[], object]) -> None:
        self._jobs.submit(
            work,
