...
from pardus_panel.gtk.periodic_refresh import PeriodicRefreshCoordinator
from pardus_panel.gtk.widgets.live_graph import LiveGraph
...
class PerformancePage:
    def __init__(self, *, jobs: AsyncJobRunner) -> None:
        ...
        builder = Builder("Performance.ui")
        ...
        self._cpu_graph = LiveGraph(colors=(CPU_COLOR,), adaptive=False, height=72)
        self._memory_graph = LiveGraph(colors=(MEMORY_COLOR,), adaptive=False, height=72)
        self._network_graph = LiveGraph(
        ...
        self._refresh = PeriodicRefreshCoordinator(
            interval_ms=2_000,
            jobs=jobs,
            work=collector.collect,
    ...
    def set_active(self, active: bool) -> None:
        self._refresh.set_active(active)
    ...
    def _replace_disks(self, disks: tuple[DiskSnapshot, ...]) -> None:
        ...
        if not cards:
            cards.append(_message_card(_("No physical disks found")))
...
def _usage_card(
    ...
    card, _value = _resource_card(name, f"{percent:.1f}%")
...
def _core_card(
    ...
    card, percent = _resource_card(name, "—")
    graph = LiveGraph(colors=(color,), adaptive=False, height=72)
    graph.get_style_context().add_class("performance-core-graph")
    card.pack_start(graph, True, True, 0)
    return card, percent, graph


def _resource_card(name: str, value: str) -> tuple[Gtk.Box, Gtk.Label]:
    ...
    value_label.get_style_context().add_class("performance-resource-percent")
