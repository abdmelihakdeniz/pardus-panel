    ...
    *,
    ...
    for line in output.splitlines():
        ...
        if entry is not None and (
            not needle or needle in f"{entry.source} {entry.message}".casefold()
        ):
...
def parse_record(line: str) -> LogEntry | None:
    ...
    if isinstance(message, list):
        message = bytes(
            value for value in message if isinstance(value, int) and 0 <= value <= 255
        )
...
def _timestamp(value: object) -> datetime | None:
    try:
        return datetime.fromtimestamp(int(str(value)) / 1_000_000, tz=timezone.utc)
