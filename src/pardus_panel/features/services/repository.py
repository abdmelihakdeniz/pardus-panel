...
UNIT_CHARACTERS = ascii_letters + digits + "@_.:-\\"
...
def list_services() -> tuple[ServiceEntry, ...]:
    ...
    for scope in ServiceScope:
        try:
            states = _read_states(scope)
            enabled = _read_enabled(scope)
        except CommandError as error:
            first_error = first_error or error
            continue
        for unit in states.keys() | enabled.keys():
...
def _read_states(scope: ServiceScope) -> dict[str, tuple[str, str, str]]:
    output = run_command(
        scope_command(
            scope,
            "list-units",
    ...
    for line in output.splitlines():
        ...
        states[unit] = description, active, sub
    return states
