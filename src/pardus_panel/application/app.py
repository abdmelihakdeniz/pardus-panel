...
PAGES = (
    ("process", ProcessPage, "view-list-symbolic", _("Processes")),
    ("performance", PerformancePage, "system-run-symbolic", _("Performance")),
    ("autostart", AutostartPage, "media-playback-start-symbolic", _("Autostart")),
    ("services", ServicesPage, "applications-system-symbolic", _("Services")),
    ("logs", LogsPage, "text-x-generic-symbolic", _("Logs")),
    ("power", PowerPage, "battery-symbolic", _("Power")),
    (
        "system-info",
        SystemInfoPage,
...
class PardusPanelApplication(Gtk.Application):
    ...
    def _install_pages(self, builder: Builder) -> None:
        ...
        for name, page_type, icon, label in PAGES:
            page = page_type(jobs=self._jobs)
            stack.add_named(page.root, name)
            self._pages_by_name[name] = page
            row = Gtk.ListBoxRow(name=name)
            content = Gtk.Box(spacing=10, margin=10)
            content.pack_start(
